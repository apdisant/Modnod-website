...
//
    ...
    $AccessToken = htmlentities($_POST["txtAccessToken"], ENT_QUOTES, "UTF-8");
    if ($debug) print '<p> Access Token: ' .$AccessToken.'</p>';
...
//
    ...
    if (empty($AccessToken)) {
        ...
        $valid = verifyAlphaNum($AccessToken); /* test for non-valid  data */
        if (!$valid) {
...
//    
        ...
        try {
            ...
            $sql .='fldAccessToken="' .$AccessToken.'"; '; 
...
<fieldset>
      ...
      <input type="password" id="txtPassword" name="txtPassword" value="<?php echo $Password; ?>" class="element text medium" placeholder="Password"  onfocus="this.select()" tabindex=2 />
                ...
                <fieldset class="contact">
                    <label class="required" for="txtAccessToken">Access Token </label>

                    <input id ="txtAccessToken" name="txtAccessToken" class="element text medium<?php if ($AccessTokenERROR) echo ' mistake'; ?>" type="text" maxlength="40" value="<?php echo $AccessToken; ?>" placeholder="Access Token" onfocus="this.select()"  tabindex=3 />
</fieldset>
